...
field_inst=csv_read('inst_field.csv')
# convert from 1D to 2D array
...
for i in range(len(field_inst[var])):
    vi[zi.index(field_inst['z'][i])][ri.index(field_inst['r'][i])]=field_inst[var][i]
    Zi[zi.index(field_inst['z'][i])][ri.index(field_inst['r'][i])]=field_inst['Z'][i]
