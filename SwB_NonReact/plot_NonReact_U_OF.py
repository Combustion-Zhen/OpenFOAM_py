...
OF_cas = ['PISO','PISO_Bulk']
OF_label = ['Turb','Bulk']
FLUENT_cas = ['Smag',]
line_style = ['-','--','-.',':']
line_color = ['tab:red','tab:green','tab:blue','tab:orange']
line_width = 1.5
...
for i in range(num_rows):
    ...
    for j, case in enumerate(OF_cas):
        ...
        axz[i,0].plot(OF_data['Points0'],OF_data['UMean2'],
                label=OF_label[j],lw=line_width,ls=line_style[k],c=line_color[k])
        axz[i,1].plot(OF_data['Points0'],np.sqrt(OF_data['UPrime2Mean2']),
                label=OF_label[j],lw=line_width,ls=line_style[k],c=line_color[k])

        axr[i,0].plot(OF_data['Points0'],OF_data['UMean0'],
                label=OF_label[j],lw=line_width,ls=line_style[k],c=line_color[k])
        axr[i,1].plot(OF_data['Points0'],np.sqrt(OF_data['UPrime2Mean0']),
                label=OF_label[j],lw=line_width,ls=line_style[k],c=line_color[k])

        axt[i,0].plot(OF_data['Points0'],OF_data['UMean1'],
                label=OF_label[j],lw=line_width,ls=line_style[k],c=line_color[k])
        axt[i,1].plot(OF_data['Points0'],np.sqrt(OF_data['UPrime2Mean1']),
                label=OF_label[j],lw=line_width,ls=line_style[k],c=line_color[k])
